const OrdersController = (sec, orderService) => {

    const router = require("express").Router();

    router.get("/orders", sec.authenticateJWT, async (req, res) => {
        const orders = await orderService.getOrdersVisibleToUser(req.user.id);
        res.json({ status: "success", data: { orders } });
    });

    router.post("/orders", sec.authenticateJWT, sec.isCustomer, async (req, res) => {
    ...
    router.put("/orders/:id", sec.authenticateJWT, sec.isDriver, async (req, res) => {
        // First, get the order
    ...
    router.get("/orders/:id", sec.authenticateJWT, async (req, res) => {
        // First, get the order
        const order = await orderService.getOrderById(parseInt(req.params.id));
        if (order) {
